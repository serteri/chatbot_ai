    ...
    try {
        ...
        const tableBody = warnings.map((warning, idx) => {
            ...
            if ((lowerWarning.includes('nominee') || lowerWarning.includes('representative') || lowerWarning.includes('rep') || lowerWarning.includes('guardian')) && !usedClauseCategories.has('nominee')) {
                ...
                activeClause = `Clause ${idx + 1}.1: Informed Consent: ${subjectName} hereby provides informed consent for the Provider (${brandName}) to collect, store, and share necessary care information strictly as required by the NDIS Act 2013.`
        ...
        doc.setFontSize(7)
        doc.setTextColor(100, 116, 139)
        doc.text('Human-in-the-loop confirmation received via NDIS Shield Hub Compliance Engine.', pageWidth - margin - 5, cursorY + 15, { align: 'right' })
        ...
        for (let i = 1; i <= totalPages; i++) {
            ...
            doc.setFontSize(7)
            doc.setTextColor(148, 163, 184)
            doc.text('NDIS Shield Hub Sovereign AI | Sydney Region (ap-southeast-2) | NDIS Compliance Engine', pageWidth / 2, pageHeight - 22, { align: 'center' })

            // Legal disclaimer - prominent
            doc.setFont('helvetica', 'bold')
            doc.setFontSize(6.5)
            doc.setTextColor(220, 38, 38) // Red-600
            doc.text('LEGAL DISCLAIMER: This document is a compliance draft generated by NDIS Shield Hub and must be verified by the provider\'s authorized officer.', pageWidth / 2, pageHeight - 17, { align: 'center' })
