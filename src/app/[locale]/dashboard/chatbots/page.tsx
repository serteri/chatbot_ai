...
import {
    ...
    Activity,
    MessageCircle,
    Settings // Ayarlar ikonu linklerde kullanılıyor
} from 'lucide-react'
...
// YENİ: Sorunlu kısmı çözecek olan Client Component'i ekliyoruz
import { ChatbotCardActions } from '@/components/chatbot/ChatbotCardActions'

export const dynamic = 'force-dynamic'

export default async function ChatbotsPage(props: {
    ...
    const whereCondition: any = {
        ...
        NOT: {
            industry: { in: ['education', 'ecommerce'] }
        },
        name: {
            contains: query,
            mode: 'insensitive'
                    ...
                    <div className="flex items-center justify-between h-16">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <span className="font-semibold text-slate-900">{t('dashboard.generalChatbots')}</span>
                            </div>
                        </div>

                        <div className="flex items-center gap-3">
                            <CreateChatbotDialog
                                prefilledData={{ type: 'general' }}
                                trigger={
                                    <Button className="bg-slate-900 hover:bg-slate-800 shadow-sm text-white transition-all hover:scale-105">
                    ...
                    <Card className="border-dashed border-2 border-slate-200 bg-slate-50/50">
                        <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                            ...
                            <p className="text-slate-500 max-w-sm mb-6 text-sm">
                                {query || statusFilter !== 'all' ? 'Filtreleme sonucunda chatbot bulunamadı.' : t('dashboard.noGeneralChatbotsDesc')}
                            </p>
                            ...
                            <Card key={chatbot.id} className="group hover:shadow-lg transition-all duration-200 border-slate-200 bg-white flex flex-col overflow-hidden">
                                <CardHeader className="pb-4 space-y-0 border-b border-slate-50 bg-slate-50/30">
                                    <div className="flex items-start justify-between">
                                        <div className="flex gap-3">
                                            <div className="h-12 w-12 rounded-xl flex items-center justify-center shadow-sm flex-shrink-0 text-white bg-slate-700">
                                                <Bot className="h-6 w-6" />
                                            </div>
                                            <div>
                                        ...
                                        {/* HATA ÇÖZÜMÜ: Buradaki eski DropdownMenu kodunu sildik ve yerine yeni Client Component'i koyduk */}
                                        <ChatbotCardActions
                                            chatbotId={chatbot.id}
                                            locale={locale}
                                            labels={{
                                                embed: 'Siteye Ekle', // Çeviri anahtarınız varsa t('...') kullanın
                                                manage: t('manage'),
                                                settings: 'Ayarlar',
                                                delete: 'Sil'
                                            }}
                                ...
                                <CardContent className="flex-1 flex flex-col pt-4">
                                    ...
                                    <div className="mt-auto flex gap-2">
                                        ...
                                        <Link href={`/${locale}/widget-test?chatbotId=${chatbot.identifier}&mode=general`} target="_blank" className="flex-1">
