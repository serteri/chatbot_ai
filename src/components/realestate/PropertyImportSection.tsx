                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                            ...
                            <div className="p-4 border rounded-lg hover:border-purple-300 hover:bg-purple-50/50 transition-colors">
                                ...
                                <Button
                                    ...
                                    draggable
                                    onDragStart={(e) => {
                                        // Minified & Inlined Bookmarklet to bypass CSP
                                        const code = `javascript:(function(){var d=document;var msg=function(t,c){var e=d.createElement('div');e.style.cssText='position:fixed;top:20px;right:20px;z-index:999999;padding:12px 20px;background:'+c+';color:white;border-radius:8px;font-family:sans-serif;box-shadow:0 10px 15px -3px rgba(0,0,0,0.1);font-weight:600;font-size:14px';e.textContent=t;d.body.appendChild(e);setTimeout(function(){e.remove()},4000)};msg('🏠 Saving property...', '#3b82f6');var get=function(s){var e=d.querySelector(s);return e?e.innerText.trim():''};var getClean=function(s){var t=get(s);return t.replace(/[^0-9.]/g,'')};var num=function(s){var m=get(s).match(/(\\d+)/);return m?parseInt(m[0]):0};var u=window.location.href;var data={sourceUrl:u,listingType:u.includes('rent')?'rent':'sale',images:[]};try{if(u.includes('realestate.com.au')){data.title=get('h1')||get('.property-address');data.price=parseFloat(getClean('[data-testid="listing-details__summary-price"]')||getClean('.property-price'));data.description=get('[data-testid="listing-details__description"]')||get('.property-description');data.address=get('[data-testid="listing-details__button-copy-address"]')||data.title;data.bedrooms=num('[data-testid*="features"] [aria-label*="bedrooms"]')||num('.bedrooms')||0;data.bathrooms=num('[data-testid*="features"] [aria-label*="bathrooms"]')||num('.bathrooms')||0;d.querySelectorAll('img').forEach(function(i){if(i.src&&i.src.includes('bucket-api')&&!i.src.includes('agent')&&data.images.length<5)data.images.push(i.src)});var parts=u.split('-');var stIdx=parts.findIndex(function(p){return ['qld','nsw','vic','wa','sa','tas','act','nt'].includes(p)});if(stIdx!==-1){data.city=parts[stIdx].toUpperCase();data.suburb=parts[stIdx+1];if(data.suburb)data.suburb=data.suburb.charAt(0).toUpperCase()+data.suburb.slice(1)}data.propertyType=u.includes('apartment')||u.includes('unit')?'apartment':'house'}else if(u.includes('domain.com.au')){data.title=get('h1');data.price=parseFloat(getClean('[data-testid="listing-details__summary-price"]'));data.address=get('h1');data.description=get('[data-testid="listing-details__description"]');data.bedrooms=num('[data-testid*="features-beds"]');data.bathrooms=num('[data-testid*="features-baths"]');data.propertyType='house';data.city='VIC';d.querySelectorAll('[data-testid*="gallery"] img').forEach(function(i){if(i.src&&data.images.length<5)data.images.push(i.src)})}else{throw new Error('Site not supported')}fetch('${window.location.origin}/api/properties/create-from-bookmarklet',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({chatbotId:'${selectedChatbotId}',...data})}).then(function(r){return r.json()}).then(function(r){if(r.success)msg('✅ Property saved!', '#10b981');else msg('❌ Error: '+(r.error||'Unknown'), '#ef4444')}).catch(function(e){msg('❌ Network Error', '#ef4444');console.error(e)})}catch(e){msg('❌ Script Error: '+e.message, '#ef4444');console.error(e)}})()`
                                        e.dataTransfer.setData('text/plain', code);
                                        e.dataTransfer.setData('application/x-bookmark', code);
                                        e.currentTarget.setAttribute('href', code); // For explicit link drag behavior
                                    }}
                                    onClick={(e) => {
                                        e.preventDefault();
                                        alert(locale === 'tr'
